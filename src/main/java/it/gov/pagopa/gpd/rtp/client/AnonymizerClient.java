...
@FeignClient(
    value = "anonymizer",
    url = "${service.anonymizer.host}",
    configuration = AnonymizerFeignConfig.class)
public interface AnonymizerClient {

  @Retryable(
      maxAttemptsExpression = "${service.retry.maxAttempts}",
      backoff = @Backoff(delayExpression = "${service.retry.maxDelay}"))
  @PostMapping(value = "${service.anonymizer.path}", consumes = MediaType.APPLICATION_JSON_VALUE)
