...
public interface FilterService {
   ...
   * @param operation the DebeziumOperationCode indicating the type of operation performed on the
   *     PaymentPosition.
   */
  void filterByStatus(PaymentPosition debtPosition, DebeziumOperationCode operation);

  /**
   * This method checks if the service type of PaymentPosition is valid for RTP processing. In
   * particular, it allows only GPD and ACA service types, excluding ACA service types with
   * paCreatePosition.
   *
   * @param debtPosition the PaymentPosition to be validated.
   */
  ...
  /**
   * This method evaluates if the PaymentOption has valid Transfer categories for RTP processing. In
   * particular, it checks that no Transfer category starts with "6/", "7/", or "8/" and is valid.
   *
