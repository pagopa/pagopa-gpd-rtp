...
class HelpdeskControllerTest {
    ...
    void getBlobList_OK() throws Exception {
        ...
        mockMvc
                .perform(get(String.format("/helpdesk/blob/list?year=%s&month=%s&day=%s&hour=%s", YEAR, MONTH, DAY, HOUR)))
    ...
    void getJSONFromBlobStorage_OK() throws Exception {
        ...
        mockMvc
                .perform(get(String.format("/helpdesk/blob/json?filename=%s", FILENAME)))
                .andExpect(status().isOk());
    ...
    void retryMessage_OK() throws Exception {
        ...
        mockMvc
                .perform(post(String.format("/helpdesk/blob/retry?filename=%s", FILENAME)))
                .andExpect(status().isOk());
