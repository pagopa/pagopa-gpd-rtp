...
class DeadLetterServiceImplTest {
    ...
    private static final String ORIGINAL_MESSAGE_PAYLOAD = "\"originalMessage\":\"[ERROR] Retrieving original message payload\"";
    ...
    void sendToDeadLetter_success_case() {
        ...
        verify(blobStorageClient).saveStringJsonToBlobStorage(errorMessageCaptor.capture(), filePathCaptor.capture());
        ...
        assertTrue(capturedErrorMessage.contains(DEAD_LETTER_CAUSE_FIELD + AppError.INTERNAL_SERVER_ERROR.getDetails()));
        assertTrue(capturedErrorMessage.contains(DEAD_LETTER_ERROR_CODE_FIELD + AppError.INTERNAL_SERVER_ERROR.name()));
    ...
    void sendToDeadLetter_error_parsing_original_key() {
        ...
        assertTrue(capturedErrorMessage.contains(DEAD_LETTER_ID_FIELD + errorMessage.getHeaders().getId()));
    }
    ...
    private ErrorMessage buildErrorMessage() {
        ...
        MessageHeaders originalMessageHeaders = new MessageHeaders(Map.of(KafkaHeaders.RECEIVED_KEY, CDC_MESSAGE_KEY));
        MessageHeaders errorMessageHeaders = new MessageHeaders(Collections.emptyMap());
        Message<byte[]> originalMessage =
                new GenericMessage<>(String.valueOf(new DataCaptureMessage<>()).getBytes(), originalMessageHeaders);

        return new ErrorMessage(new MessageHandlingException(originalMessage, appException), errorMessageHeaders, originalMessage);
    }
